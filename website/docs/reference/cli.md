...
- **Removed files:** TioAnime `YourUpload` files are deleted per-episode upstream (`novideo.mp4`); when that happens the error names sibling servers (Mega/Voe, Netu, VidGuard) with browser URLs, and suggests another episode or JKAnime. Mega/Voe/Netu embeds are browser-only (no direct resolver). **Auto-skip:** when a selected episode fails with this error, ani-cli-rs automatically tries the next episode and prints a warning; if it was the last episode the original error is returned.
